...
public class UpdatingScreen extends GuiScreen {
    ...
    private static final int DOT_TIME = 200;  // ms entre "." -> ".." -> "..."
    ...
    private void updateText() {
        backButton.displayString = (failed || complete) ? "OK" : "Annuler";
    }
    ...
    private void doUpdate(boolean restartNow) {
        try {
            ...
            }, "Fil de téléchargement de mise à jour NEC").start();
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
    ...
    private void downloadUpdate(String url, File directory) {
        try {
            ...
            if (st.getResponseCode() != HttpURLConnection.HTTP_OK) {
                failed = true;
                updateText();
                Reference.logger.error(url + " code d'état renvoyé " + st.getResponseCode());
                return;
            }
            ...
            if (!directory.exists() && !directory.mkdirs()) {
                failed = true;
                updateText();
                Reference.logger.error("Impossible de créer le répertoire du fichier de mise à jour");
                return;
            }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (failed) {
            drawCenteredString(Minecraft.getMinecraft().fontRendererObj, EnumChatFormatting.RED + "Le téléchargement de la mise à jour a échoué", this.width / 2, this.height / 2, 0xFFFFFFFF);
        } else if (complete) {
            drawCenteredString(Minecraft.getMinecraft().fontRendererObj, EnumChatFormatting.GREEN + "Téléchargement de la mise à jour terminé", this.width / 2, this.height / 2, 0xFFFFFF);
        } else {
            ...
            int x = (this.width - Minecraft.getMinecraft().fontRendererObj.getStringWidth(String.format("Téléchargement %s", DOTS[DOTS.length - 1]))) / 2;
            String title = String.format("Téléchargement %s", DOTS[((int) (System.currentTimeMillis() % (DOT_TIME * DOTS.length))) / DOT_TIME]);
